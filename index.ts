...
//cau 1
let tth1: HTMLElement = document.querySelector('h1.titleH1')
...
let found = true
let click: HTMLElement = document.querySelector('button.btnClick')
...
const userConfirmed = window.confirm("Bạn có muốn khôi phục lại dữ liệu đã lưu không?");
...
if (userConfirmed) {
    ...
    if (savedTasks) {
        ...
        for (let i = 0; i < savedTasks.length; i = i + 1){
            let newtask = `<p><input type="checkbox">${savedTasks[i]}</p>`
            listtask.innerHTML =  listtask.innerHTML + newtask;
